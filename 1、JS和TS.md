...
## 18.浏览器事件循环
    1.JS调用栈采用的是后进先出的规则，当函数执行的时候，会被添加到执行栈，当执行栈执行完成后，就会从栈中弹出。异步任务进入Event Table并注册函数。当指定的事情完成时，Event Table会将这个函数移入Event Queue（事件队列）。
    2.同步任务会在调用栈中按照顺序等待主线程依次执行，异步任务会在同步任务执行完，调用栈被清空后，读取到执行栈执行。
