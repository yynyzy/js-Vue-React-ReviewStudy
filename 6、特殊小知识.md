...
# 14.cookie和session的方式有很多的缺点︰
4.Cookie会被附加在每个HTTP请求中，所以无形中增加了流量（事实上某些请求是不需要的);
2.Cookie是明文传递的，所以存在安全性的问题;
3.Cookie的大小限制是4KB，对于复杂的需求来说是不够的;
4.对于浏览器外的其他客户端（比如iOS、Android )，必须手动的设置
5.cookie和session 对于分布式系统和服务器集群中如何可以保证其他系统也可以正确的解析session ?
