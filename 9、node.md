...
# 3.node 多进程
js是脚本语言不能直接被识别的，需要浏览器引擎去通过一系列词法分析转成 AST树 在经过中间一系列优化传成机器码。为了使js在服务端环境运行 我们node引入了v8引擎。
当代码存在异步操作的时候 引擎将异步代码移出调用栈放入一个事件队列里面 然后继续执行后续程序 eventloop尝试用libuv线程池里面取出一个空闲的线程去执行队列里面的操作 执行完毕获得结果通知主线程 主线程执行相关回调函数 并将线程归还给线程池。
...
2.用这个的前提是每个监听这个端口的进程，监听的文件描述符要相同（每个子进程收到的文件描述符都是一样的，这个时候就是了）
（
...
# 6.对项目进行 HTTP压力测试
常见的使用 ApacheBench（ab）进行网络压力测试
启动项目后，使用命令：
```
ab -c200 -n1600 http://127.0.0.1:3000
```
-c200 是指同时模拟200台 client 发送请求
-n1600 是指发持续送1600次请求
http://127.0.0.1:3000 是指运行的项目所在的ip地址

主要通过 Request per Second（QPS 每秒平均并发量）